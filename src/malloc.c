...
static struct _block *LastAllocation = NULL;
#endif
...
// \TODO Put your Next Fit code in this #ifdef block
   ...
   {
      curr = heapList;
      while (curr && !(curr->free && curr->size >= size) && curr != LastAllocation) 
      {
         *last = curr;
         curr  = curr->next;
      }
   }
   if(curr == LastAllocation)
   {
      curr = NULL;
   }
   else
...
void *malloc(size_t size) 
{
   ...
   */
   if(next != NULL)
   {
      if(next->size > size)
      {
         int left = next->size - size;
         if(left >= sizeof(struct _block)+4)
         {
            struct _block *split = (struct _block *)((char *)next + sizeof(struct _block) + size);
            split->size = left - sizeof(struct _block);
            split->free = true;
